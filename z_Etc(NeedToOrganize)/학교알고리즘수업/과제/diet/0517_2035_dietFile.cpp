...
//				cout << "!!! " << sum1[j] << "!!! ";
			...
			/*for(int j=0; j<4; j++){
				...
				if(sum1[j] >= goal[j]){
					...
					}//It is for the setting a first best. // I should fix this part. Problem is that If it satisfy goal, It would not find best combination. just use first satisfying combination.
					if(j==3){
						cout << "check0" << endl;
						if(best[4] > sum1[4]){
							for(int temp=0; temp<5; temp++){
