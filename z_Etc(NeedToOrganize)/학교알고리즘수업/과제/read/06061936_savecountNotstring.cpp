...
private:
	string ngs;
	vector<string> array;
	list<int> alphabet[27]; // alphabet26 + 1(for NULL)
...
void Read::radixSort(){
	...
	for(int lineNum=0; lineNum<array.size(); ++lineNum){ // countSort, sNum = string Number, cNum = character Number 
		if(array[lineNum].size()-1 < 49){ // if array size = 5, bound is array[4];
			//cout << "check01" << endl;
			alphabet[26].push_back(lineNum); 
		}
		else{
			//cout << "check02" << endl;
		//	cout << array[lineNum][loc] << endl;
	//		cout << array[lineNum][loc] - 97 << endl;
			alphabet[array[lineNum][49] - 97].push_back(lineNum);
			}
		}
	for(int i=0; i<27; i++){
		...
		for(int aList=0; aList<27; aList++){
			...
			for(int i=0; i<aCount[aList]; i++){
				if(alphabet[i].size()-1 < loc){
					...
					cout << "check55" << endl;
					alphabet[array[alphabet[i].front()][loc]-97].push_back(alphabet[i].front());
					
					//cout << alphabet[array[alphabet[0].front()][0]-97].front() << endl;
	...
	for(int aLoc=0; aLoc<27; aLoc++){
		for(int aList=0; aList < alphabet[aLoc].size(); aList++){
			array[temp++] = array[alphabet[aLoc].front()];
...
int main(void){
	...
	alphabet[0].push_back(0);
	alphabet[0].push_back(1);
	alphabet[0].push_back(1);
	
	array.push_back("amynameispdg");
	cout << "!!!!!!!!!!"<<endl;
	//cout << alphabet[ (array[alphabet[0].front()][0]-97) ] << endl;
	cout << alphabet[array[alphabet[0].front()][0]-97].front() << endl;
	alphabet[array[alphabet[0].front()][0]-97].push_back(2);
	
	
	cout << alphabet[0].front() << endl;
