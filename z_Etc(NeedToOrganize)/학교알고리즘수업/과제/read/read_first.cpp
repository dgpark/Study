...
void countSort(string array[], int size1, size_t loc){
    ...
    for (int j = 0; j <size1; j++){   
        count[loc < array[j].size() ? (int)(unsigned char)array[j][loc] + 1 : 0]++;
    }
    ...
    for (int r = size1 - 1; r >= 0; r--){
        array2[count[loc < array[r].size() ? (int)(unsigned char)array[r][loc] + 1 : 0] - 1] = array[r];
        count[loc < array[r].size() ? (int)(unsigned char)array[r][loc] + 1 : 0]--;
    }
